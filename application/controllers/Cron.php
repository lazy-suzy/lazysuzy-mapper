...
class Cron extends CI_Controller
{
    ...
    public function index($filter_check = null)
    {
        ...
        if (isset($status['category'])) {
            ...
            log_message('error', '[INFO | START] Cron.php index');
            ...
            foreach ($urls as $key => $url) {
                ...
                foreach ($API_products as $sku => $product) {
                    ...
                    if (isset($product_details)) {
                        $image_links   = $this->multiple_download($product_details->SecondaryImages, '/var/www/html/cb2/_images/main', '/cb2/_images/main/');
                    ...
                    $fields = array(
                        ...
                        'online_msg'       => isset($product_details->Availability->OnlineMessage) ? $this->tr($product_details->Availability->OnlineMessage) : ""
                    ...
                    if (!in_array($product_details->SKU, $harveseted_SKU)) {
                        ...
                        if (isset($ss[0])) {
                            $aa['is_back_order'] = isset($product_details->Availability->IsBackOrdered) ? $product_details->Availability->IsBackOrdered : "";
                            $aa['back_order_msg'] = isset($product_details->Availability->BackOrderedMessage) ? $product_details->Availability->BackOrderedMessage : "";
                            $aa['back_order_msg_date'] = isset($product_details->Availability->BackOrderedMessageDate) ? $product_details->Availability->BackOrderedMessageDate : "";
                            $aa['online_msg'] = isset($product_details->Availability->OnlineMessage) ? $product_details->Availability->OnlineMessage : "";
                        }
    ...
    {
        $to_replace = "In stock and ready for delivery to ZIP code";
        $replace_with = "In stock and ready for delivery";
        return str_replace($to_replace, $replace_with, $str);
    }
