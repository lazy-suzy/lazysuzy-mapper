...
class Common extends CI_Controller {
	...
	function insert_data($array, $site_id)
	{
		foreach($array as $department => $category)
		{
			foreach($category as $key => $product_category)
			{				
				echo $department.' = '.$product_category.' = '.$site_id; echo '<br>';
				...
				if($product_sku!='' && $product_url!='')
				{
					...
					{	
						...
						if($description!=NULL)
						{
							...
							if($update)
							{
								echo "Description details updated successfully into ".$Row->product_sku; echo "<br>";
							}
							else
							{
								echo "error".$Row->product_sku; echo "<br>";
							}
						}
						else
						{
							echo "NULL description found ".$Row->product_sku; echo "<br>";
						}
					}
				}
			}
