...
//      if( !data.key ){
	...
	toString: function() {
		//return "DynaTreeNode<" + this.data.key + ">: '" + this.data.chk + "'";
		
		//$('#selected_cat_id').val(this.data.sel_cat_id);
		
		$('#own_cat_id').val(this.data.cat_id);
...
//              this.tree.logDebug("nodeTitle: " + nodeTitle);
				...
				var mapped_cat_class = '';				
				if(data.ismapped == '0')
				var mapped_cat_class = 'mappcat';
				
				var vAddlink = null;
				if( data.isLazy == true && data.sel_cat_id > 0  )
				{
					vAddlink = 'add all';
				}
				else if( data.isFolder == true && data.sel_cat_id != undefined )
				{
					vAddlink = 'add all';						
				}
				else if( data.isFolder == false && data.sel_cat_id != undefined )
				{
					vAddlink = 'add';						
				}
				else
				...
				var vItemType = data.item_type;
				
				if(vAddlink!='')
				optionstr = '<a id="cat-add-'+data.sel_cat_id+'" href="javascript:void(0)" class="current_cat_option" onclick="category_option_area(\''+vItemType+'\',\''+data.sel_cat_id+'\')" style="color:#01A0DA;font-size:11px">'+vAddlink+'</a>';
								
				// condition for Google Search Link
				if(typeof(data.title) !== 'undefined' && data.title != null){
					vdatatitle = data.title.replace(/'/g, "");
				}
				else{
					vdatatitle = data.title;
				}
				var googlesrch = '<a style="color:#22878E" href="javascript:;" onclick="googlesrchfn(event,\''+vdatatitle+'\')">(link)<a>';
				
				nodeTitle = '<a id="cat-id-'+data.sel_cat_id+'" href="' + href + '" class="'+mapped_cat_class+' current_cat ' + opts.classNames.title + '"' + tooltip + '>' + data.title + '</a>' +optionstr ;
			}
		}
		
		if(!isNaN(data.sel_cat_id) && data.googlesrch == '1' )  // condition for Google Search Link
		res += nodeTitle+googlesrch;
		else
		res += nodeTitle;
...
//              this.span.innerHTML = this._getInnerHtml();
			...
			//Mariraj	
			/*if( !isNaN(data.ismapped)){				
				
				var hideclass = ' hideclass';
				cnList.push(hideclass);
			}*/
...
//      this.tree.logDebug("%s.addChild(%o, %o)", this, obj, beforeNode);
	...
	appendAjax: function(ajaxOptions) {		
		ajaxOptions.data['parent_cat_id'] = $('#own_cat_id').val();
		ajaxOptions.data['camp_vdoad_id'] = $('#var_camp_vdoad_id').val();
...
//              self.tree.logDebug("appendAjax().success");
				...
				// Category Mapping Hidding // Mariraj
				if(ajaxOptions.data.mode=='box1')
				{
					 self.visit(function(node){						   
						 if(node.data.ismapped=='0')  
						 {
							 $('#tree_drag1 #cat-id-'+node.data.cat_id).parent('span').parent('li').attr('data-added','1');
						 	 $('#tree_drag1 #cat-id-'+node.data.cat_id).parent('span').parent('li').hide();
						 }
						 else{
							 $('#tree_drag1 #cat-id-'+node.data.cat_id).parent('span').parent('li').attr('data-added','0'); 
						 }
					 });						 
				}
...
//          this._setDndStatus(otherNode, node, ui.helper, hitMode, res!==false);
		...
		case "drop":
			...
			{
				register_mapping_category();
				this._chkAlreadtExist();
			}
			
			
			// Mariraj 
			var drapnodeval = $('#drag_cat_id').val();
			if($('#hidden_mapping_type').val()=='category')
			$('#tree_drag1 #cat-id-'+drapnodeval).parent('span').parent('li').hide();	
